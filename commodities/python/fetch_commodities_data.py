...
def get_commodity_data(ticker_symbol, period='3mo', interval='1d'):
    """
    Fetch commodity data with all available parameters and technical indicators

    Args:
        ticker_symbol: Commodity ticker symbol (e.g., 'GC=F', 'SI=F')
...
def save_to_csv(data, ticker_symbol, output_dir):
    ...
    # Generate filename
    filename = f"{ticker_symbol.replace('=', '').replace('^', '')}_data.csv"
...
def main():
    """Main function to fetch and save commodity data"""
    # Configuration
    ticker_symbols = [
        'GC=F',          # Gold Futures
        'SI=F',          # Silver Futures
    ...
    for ticker in ticker_symbols:
        ...
        try:
            # Fetch data
            data = get_commodity_data(ticker, period=period, interval=interval)
