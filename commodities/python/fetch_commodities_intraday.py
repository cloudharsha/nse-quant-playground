"""
Fetch intraday commodities data using period parameter
"""
...
def get_extended_intraday_data(ticker_symbol, period='1mo', interval='5m'):
    """
    Fetch intraday data using period parameter

    Args:
        ticker_symbol: Commodity ticker symbol (e.g., 'GC=F')
...
def save_to_csv(data, ticker_symbol, interval, output_dir):
    ...
    # Generate filename
    filename = f"{ticker_symbol.replace('=', '').replace('^', '')}_data_{interval}.csv"
...
def main():
    """Main function to fetch and save intraday commodities data"""
    # Configuration
    ticker_symbols = [
        'GC=F',          # Gold Futures
        'SI=F',          # Silver Futures
    ...
    period = '1mo'  # 1 month (30 days - maximum working period)
    intervals = ['5m', '15m', '30m', '1h']  # Multiple timeframes
    output_dir = '/mnt/c/Users/harsh/Desktop/workspace/git/nse-quant-playground/commodities/data'

    print("=" * 60)
    print("Commodities Intraday Data Collection")
    print("=" * 60)
    print(f"Period: {period} (30 days - maximum working period)")
    print(f"Intervals: {', '.join(intervals)}")
    print(f"Output directory: {output_dir}")
    print(f"Number of commodities: {len(ticker_symbols)}")
    print("=" * 60)
    print("Note: Yahoo Finance limits intraday data to maximum 60 days")
    print("=" * 60)

    # Fetch data for each commodity and each interval
