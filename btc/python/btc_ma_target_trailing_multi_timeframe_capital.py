...
TARGET_RULE = "fixed_2r_from_entry_to_initial_ma_stop_with_trailing_ma_stop"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="BTC local multi-timeframe MA target backtest with fixed-notional capital."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
...
def run_backtest(
    ...
    logger = base.configure_logger(
        ...
        f"btc_ma_target_trailing_multi_timeframe_capital_{spec.timeframe}",
    )
    ...
    logger.info(
        ...
        spec.timeframe,
        len(trade_results),
        sum(1 for result in trade_results if result.exit_reason == "target_hit_2r"),
        len(gap_events),
        len(candidate_days),
    )
...
def summarize_trades(results: list[TradeResult], capital: float) -> dict[str, Any]:
    ...
    return {
        ...
        "target_exits": sum(1 for result in results if result.exit_reason == "target_hit_2r"),
    ...
    for day in report_days:
        ...
        rows.append(
            DayResult(
                ...
                target_exits=str(
                    sum(1 for result in day_trades if result.exit_reason == "target_hit_2r")
                ),
                gap_exits=str(
...
def build_timeframe_summary_markdown(
    ...
    lines: list[str] = [
        f"# BTC {run.spec.timeframe} MA{run.spec.ma_period} 2R Target + Trailing MA Stop Backtest",
        "",
        "## Strategy Details",
        "",
        ...
        f"- Entry rule: close above SMA -> long, close below SMA -> short, and entry requires close within `{format_pct(MAX_ENTRY_GAP_PCT)[:-2] if format_pct(MAX_ENTRY_GAP_PCT).endswith('00') else format_pct(MAX_ENTRY_GAP_PCT)}`% of the SMA.",
        f"- Target rule: fixed `{format_amount(TARGET_RISK_REWARD)}R` target from the entry-to-initial-MA-stop distance.",
...
def write_timeframe_outputs(
    ...
    summary_payload = {
        ...
        "strategy": "BTC local multi-timeframe 2R target with trailing MA stop fixed-notional capital backtest",
...
def write_top_level_summary_markdown(
    ...
    lines: list[str] = [
        ...
        f"- Target is fixed at `{format_amount(TARGET_RISK_REWARD)}R` from the entry-to-initial-stop distance.",
        f"- Entry requires price to be within `{format_amount(MAX_ENTRY_GAP_PCT)}%` of the moving average.",
...
def main() -> None:
    ...
    run_name = args.run_name.strip() or (
        f"btc_ma_target_trailing_multi_timeframe_capital_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
